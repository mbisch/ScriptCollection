...
class AddItemToProject():
    ...
    def ModifyProj(self,XmlTree,file,action):
        self.Log('ModifyProj ' + str(file) + action)
        root = XmlTree.getroot()
        ns = root.tag[:root.tag.find('Project')]
        file_path = os.path.split(file)[0]
        self.Log('ModifyProj2 ')            
        if action == 'add':
            self.Log('ModifyProj3 ')              
            maxbuildorder = 0;
            ...
            #self.log("Check in "+str(file))
            for cpp in self.incl_cpp:
                rel_cpp_path = os.path.relpath(cpp,file_path)
            
                for fld in root.iter(ns + 'CppCompile'):
                    if fld.attrib['Include'].find('DbDescZusatz.cpp') >= 0:
                        self.log("Found")
                        root.remove(fld)
        self.log("End")
    ...
    def Run(self,action):
        ...
        for file in self.projs:
            ...
            if not failed:
                self.ModifyProj(tree, file, action)
    ...
    def StartGui(self):
        ...
        action_elements = []
        action_elements += [{'Name': 'Run1', 'Type': 'Action', 'Label': None, 'Action': 'Run Add Script', 'Function': self.Add, 'StdoutName': 'TextPad', 'OutQueue' : self.txt_queue}]
        action_elements += [{'Name': 'Run1', 'Type': 'Action', 'Label': None, 'Action': 'Run Delete Script', 'Function': self.Delete, 'StdoutName': 'TextPad', 'OutQueue' : self.txt_queue}]
