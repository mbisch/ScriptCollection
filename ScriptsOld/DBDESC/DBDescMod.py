...
def insert_zusatz(dbdesc_line,dirname,filename,_zusatz_map):
   ...
   if ((length < 8) or (dbdesc_line[6].find(macro_name)>=0) or (dbdesc_line[7].find('szNameLanguage2')>=0)):
      return
   
   key1 = dbdesc_line[0].strip()
   key2 = dirname
   key3 = key_from_filename(filename)
   
   IsTitle = (dbdesc_line[6].find('szName')>=0)
   dbdesc_line.insert(7,'')
   ...
   for i in range(length+1):
      ...
      if (i==7):
         ...
         if (not IsTitle):
            #Get Zusatz
            if (key1 in _zusatz_map):
               if (len(_zusatz_map[key1]) == 1 and len(_zusatz_map[key1][list(_zusatz_map[key1].keys())[0]]) == 1):
                  #print ('found unique1:' + key1)
                  szLanguage2 = list(_zusatz_map[key1][list(_zusatz_map[key1].keys())[0]].values())[0]      
               elif(key2 in _zusatz_map[key1]):
                  if (len(_zusatz_map[key1][key2]) == 1):    
                     szLanguage2 = list(_zusatz_map[key1][key2].values())[0]
                     #print ('found unique12:' + key1 + '|' + key2)
                  elif(key3 in _zusatz_map[key1][key2]):
                     szLanguage2 = _zusatz_map[key1][key2][key3]
                     #print ('found unique123:' + key1 + '|' + key2 + '|' + key3)
            if (szLanguage2):
               dbdesc_line[i] = szLanguage2+')'
            else:
               dbdesc_line[i] = '_T(""))' 
               #print ('nothing:' + key1 + '|' + key2 + '|' + key3)
         else:
            #Title
            dbdesc_line[i] = 'szNameLanguage2'        
   ...
   for dirname, dirnames, filenames in os.walk(StartDirectory):  
      ...
      for filename in filenames:
         ...
         if filename not in exclude_list:         
            filename_split = os.path.splitext(filename.upper())
            if (re.search('DBDESC',filename_split[0])) and (re.search('ZUSATZ',filename_split[0])) and (filename_split[1]=='.CPP'):
               analyze_zusatz(dirname,filename,zusatz_map)
